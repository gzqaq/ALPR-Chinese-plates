...
def generate_mask(model: CLIPSegForImageSegmentation, processor: CLIPSegProcessor, images: Sequence[np.ndarray], threshold: float = 0.7) -> np.ndarray:
  if images[0].ndim == 2:
    images = torch.tensor(images[None, ...]).to(model.device)
  else:
    images = torch.tensor(images).to(model.device)

  inputs = processor(text=["license plate"] * len(images),
                     images=images,
                     padding="max_length",
...
def detect(images: Sequence[np.ndarray], model: CLIPSegForImageSegmentation, processor: CLIPSegProcessor, threshold: float = 0.7) -> Sequence[np.ndarray]:
  if images[0].ndim == 2:
    images = [images]
  plates = []
  ...
  for image in images:
    ...
    # Inference twice
    cropped = crop_image_by_mask(cropped, generate_mask(model, processor, np.array(cropped), threshold))
...
def recognize(plate_images: Sequence[np.ndarray], decode_model: OCR, variables: Dict[str, FrozenDict], vocab: Vocabulary):
  if plate_images[0].ndim == 2:
    plate_images = [plate_images]

  encoded = decode_model.apply(variables,
  ...
  for _ in range(9):
    logits, cache = decode_model.apply({"params": variables["params"],
                                        "cache": cache["cache"]},
                                        encoded, None, targets,
                                        mutable=["cache"],
