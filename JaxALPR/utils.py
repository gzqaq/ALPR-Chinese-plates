...
class Vocabulary(object):
  ...
  def batch_encode(self, b_chars: List[str]) -> List[List[int]]:
    ...
    return list(map(lambda x: [0] + x + [1] * (self._max_len - 1 - len(x)),
                    res))
...
def parse_flags_to_model_config(
    config: ConfigDict,) -> Tuple[ModelConfig, ModelConfig, ModelConfig]:
...
def crop(image: np.ndarray,
         lp_coords: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
