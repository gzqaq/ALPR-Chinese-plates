...
def train_ocr(config: ConfigDict,
              rng: KeyArray) -> Tuple[TrainState, MetricType]:
  ...
  def _evaluate(variables, batch):
    inps, labels = batch
    logits = OCR(config.model.replace(deterministic=True)).apply(variables,
                                                                 inps,
                                                                 labels,
                                                                 mutable=False)
  ...
  for i_epoch in range(config.n_epochs):
    ...
    runner_state, metrics = jax.lax.scan(_update_minibatch, (state, rng),
                                         minibatches)
    state, rng = runner_state
    total_metrics["train_loss"].append(metrics["loss"])

    val_inp, val_labels = val_sampler.sample()
    val_batch = jnp.array(val_inp[0],
                          config.model.dtype), jnp.array(val_labels[0])
    total_metrics["val_loss"].append(
        jax.jit(_evaluate)({
            "params": state.params,
            "batch_stats": state.batch_stats
        }, val_batch))
    ...
    logging.info(
        "Epoch %d: train_loss %f, val_loss %f",
        i_epoch,
        np.mean(metrics["loss"]),
        total_metrics["val_loss"][-1],
    )
...
def train_wpod(config: ConfigDict,
               rng: KeyArray) -> Tuple[TrainState, MetricType]:
  def _update_minibatch(runner_state: Tuple[TrainState, KeyArray],
                        batch: BatchType):
    state, rng = runner_state
  ...
  train_sampler = WPODSampler(config.train_batch_size, ds[:train_split], True,
                              config.model.dtype)
  val_sampler = WPODSampler(config.val_batch_size, ds[train_split:val_split],
                            True, config.model.dtype)
