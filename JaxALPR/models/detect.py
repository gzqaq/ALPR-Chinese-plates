...
class DetectBlock(nn.Module):
  ...
  def __call__(self, inp: jax.Array):
    ...
    clsf = conv(features=2, kernel_size=(3, 3), strides=1, padding="SAME")(inp)
    clsf = nn.softmax(clsf, axis=-1)
    affine = conv(features=6, kernel_size=(3, 3), strides=1,
                  padding="SAME")(inp)
...
def wpod_loss(pred: jax.Array, label: jax.Array) -> jax.Array:
  ...
  def log_loss(pred, label):
    loss = -label * jnp.log(jnp.clip(pred, ZERO, 1.0))
    return jnp.mean(loss, axis=0)

  prob_loss = log_loss(obj_pred, obj_label) + log_loss(bg_pred, bg_label)

  affine_1 = jnp.concatenate(
      [nn.relu(pred[..., 2:3]), pred[..., 3:4], pred[..., 4:5]], axis=-1)
  affine_2 = jnp.concatenate(
      [pred[..., 5:6], nn.relu(pred[..., 6:7]), pred[..., 7:8]], axis=-1)
  affine = jnp.concatenate([affine_1, affine_2], axis=-1)

  unit = jnp.array([[-0.5, -0.5, 1], [0.5, -0.5, 1], [0.5, 0.5, 1],
                    [-0.5, 0.5, 1]])
