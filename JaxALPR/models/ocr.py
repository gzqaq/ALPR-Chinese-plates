...
class Decoder(nn.Module):
  ...
  def __call__(
    ...
    for lyr in range(config.num_layers):
      y = EncDec1DBlock(config,
                        name=f"enc_dec_block_{lyr}")(y, encoded, dec_mask,
                                                     enc_dec_mask)
...
class OCR(nn.Module):
  ...
  def decode(
    ...
    logits = self.decoder(encoded, targets, targets_pos, dec_mask, enc_dec_mask)
  ...
  def __call__(
    ...
    return self.decode(encoded, inp, targets, targets_pos, inp_seg, targets_seg)
...
if __name__ == "__main__":
  ...
  rng, _rng = jax.random.split(rng)
  output = ocr.apply(variables, dummy_inp, targets, rngs={"dropout": _rng})
