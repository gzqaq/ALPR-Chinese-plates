...
class ResBlock(nn.Module):
  ...
  def __call__(self, inp: jax.Array, train: bool = True) -> jax.Array:
    ...
    x = conv(self.n_features, kernel_size=(3, 3), strides=1,
             padding="SAME")(inp)
    x = nn.relu(norm()(x))
    x = conv(self.n_features, kernel_size=(3, 3), strides=1,
             padding="SAME")(inp)
...
class AddPosEmb(nn.Module):
  ...
  def __call__(self, inp: jax.Array, inp_pos: Optional[jax.Array] = None):
    ...
    pos_emb = sinusoidal_init(max_len=config.max_len)(pos_emb_shape,
                                                      config.dtype)
    pe = pos_emb[:, :length, :]
    ...
    if self.decode:
      ...
      if is_initialized:
        ...
        pe = jax.lax.dynamic_slice(pos_emb, jnp.array((0, i, 0)),
                                   (1, 1, d_feature))
