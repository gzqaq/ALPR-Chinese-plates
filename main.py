...
flags.DEFINE_string("clipseg_ckpt_dir", "ckpts/clipseg", "Path to the directory storing CLIPSeg checkpoint")
flags.DEFINE_string("ocr_ckpt_dir", "ckpts/ocr", "Path to the directory storing OCR checkpoint")
flags.DEFINE_integer("batch_size", 3, "Inference batch size (number of images to recognize one time)")
...
def load_model(rng: KeyArray):
  ...
  # Detection
  device = torch.device("cpu") if FLAGS.gpu == -1 else torch.device(f"cuda:{FLAGS.gpu}")
  processor = CLIPSegProcessor.from_pretrained(FLAGS.clipseg_ckpt_dir)
  clipseg = CLIPSegForImageSegmentation.from_pretrained(FLAGS.clipseg_ckpt_dir).to(device)
  ...
  *_, decode_config = parse_flags_to_model_config(get_config())
  ...
  variables = ocr.init(_rng, jnp.ones([FLAGS.batch_size, 48, 144, 3]), jnp.ones([FLAGS.batch_size, 9]))

  tx = optax.chain(optax.clip_by_global_norm(1.01),
                   optax.adam(5e-5))
  state = TrainState.create(apply_fn=ocr.apply,
                            params=variables["params"],
                            tx=tx,
                            batch_stats=variables["batch_stats"])
  state = checkpoints.restore_checkpoint(FLAGS.ocr_ckpt_dir, state)
  decode_variables = {"params": state.params,
  ...
  return {"detect": {"model": clipseg, "processor": processor},
          "ocr": {"decode_model": ocr, "variables": decode_variables, "vocab": vocab}}
...
def run_interactive(models):
  ...
  print("Enter paths to image or directory to recognize (separated by space), or 'quit' to end the program.")
  try:
    while True:
      comm = input(">>> ")
